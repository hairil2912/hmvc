...
namespace App\Modules\Data_tables_ajax\Controllers;

use App\Modules\Data_tables_ajax\Models\DataTablesAjaxModel;
...
class Data_tables_ajax extends \App\Controllers\BaseController
{
	...
	public function add()
	{
		...
		$this->view('image-upload-form.php', $data, true);
	}
	...
	public function getDataDT()
	{
		...
		foreach ($query['data'] as $key => &$val) {
			...
			$val['ignore_search_action'] = btn_action([
				'edit' => ['url' => $this->config->baseURL . $this->currentModule['nama_module'] . '/edit?id=' . $val['id_mahasiswa']], 'delete' => [
					'url' => '', 'id' =>  $val['id_mahasiswa'], 'delete-title' => 'Hapus data mahasiswa: <strong>' . $val['nama'] . '</strong> ?'
				]
			]);
			$no++;
		}
