...
namespace App\Modules\Builtin\User\Controllers;

use App\Modules\Builtin\User\Models\UserModel;
use \Config\App;
...
class User extends \App\Controllers\BaseController
{
	...
	public function index()
	{
		...
		$this->setData();
		$this->view('result.php', array_merge($data, $this->data));
	}
	...
	public function getDataDT()
	{
		...
		foreach ($query as $key => &$val) {
			...
			if ($this->actionUser['delete_data'] == 'own' || $this->actionUser['delete_data'] == 'all') {
				$btn['delete'] = [
					'url' => $this->moduleURL, 'id' =>  $val['id_user'], 'delete-title' => 'Hapus data user: <strong>' . $val['nama'] . '</strong> ?'
				];
			}
					...
					$validation->setRules(
						[
							'email' => [
								...
								'errors' => [
									'is_unique' => 'Email sudah digunakan', 'valid_email' => 'Alamat email tidak valid'
								]
							]
						]
			...
			$validation->setRules(
				[
					...
					'ulangi_password' => [
						...
						'errors' => [
							'required' => 'Ulangi password tidak boleh kosong', 'matches' => 'Ulangi password tidak cocok dengan password'
						]
					]
				]
	...
	public function edit_password()
	{
		...
		$this->view('form-edit-password.php', $this->data);
	}
}
