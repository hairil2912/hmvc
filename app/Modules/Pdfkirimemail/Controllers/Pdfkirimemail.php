...
namespace App\Modules\Pdfkirimemail\Controllers;

use App\Modules\Pdfkirimemail\Models\PdfkirimemailModel;
	...
	public function pdf()
	{
		...
		if (!empty($_POST['email'])) {
			...
			if ($send_email['status'] == 'ok') {
				...
				$message['message'] = 'Kartu elektronik gagal dikirim ke alamat email: ' . $_POST['email'] . '<br/>Error: ' . $send_email['error'];
			}
	...
	public function getDataDT()
	{
		...
		foreach ($query as $key => &$val) {
			...
			$val['ignore_search_action'] = btn_action([
				'pdf' => [
					'url' => $this->config->baseURL . $this->currentModule['nama_module'] . '/pdf?id=' . $val['id_mahasiswa'], 'btn_class' => 'btn-danger mr-1', 'icon' => 'fas fa-file-pdf', 'text' => 'PDF'
				],
				'Email' => [
					'url' => '#', 'btn_class' => 'btn-primary mr-1 kirim-email', 'icon' => 'fas fa-paper-plane', 'text' => 'Email', 'attr' => ['data-id' => $val['id_mahasiswa'], 'data-email' => $val['email'], 'target' => '_blank']
				]
